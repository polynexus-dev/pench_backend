...
def deduct_stock_on_delivery(order):
    ...
    for item in order.items.select_related('product', 'product__raw_material', 'product__bottle_type').all():
        product = item.product
        raw_mat = product.raw_material
        
        # If product is not associated with a raw material, no warehouse stock deduction is tracked
        if not raw_mat:
            logger.info(f"Product '{product.name}' has no raw material link. Skipping stock deduction.")
            continue

        # Convert quantity to raw material units based on bottle size (ml to L)
        volume_ml = product.bottle_type.volume_ml if (product.bottle_type and product.is_returnable) else 1000
