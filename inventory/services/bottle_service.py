...
from inventory.models import (
    BottleType, BottleTransaction, BottleTransactionType, CustomerBottleBalance
)

logger = logging.getLogger(__name__)

@transaction.atomic
def record_bottle_transaction(bottle_type, quantity, transaction_type, customer=None, order=None, user=None):
    """
...
@transaction.atomic
def record_bottle_return(customer, bottle_type, quantity, warehouse=None, order=None, driver=None, notes=""):
    """
    ...
    logger.info(f"Recorded return of {quantity} {bottle_type.name} from {customer.name}")
