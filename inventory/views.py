...
from .serializers import (
    ...
    CustomerProductPriceSerializer, StockMovementSerializer
)
...
class WarehouseViewSet(viewsets.ModelViewSet):
    ...
    def forecast(self, request, pk=None):
        ...
        stock_qs = Stock.objects.filter(warehouse=warehouse).select_related('raw_material')
        ...
        def calculate_demand(target_date):
            ...
            routes = Route.objects.filter(warehouse=warehouse, delivery_date=target_date)
            if routes.exists():
                order_ids = []
                for r in routes:
                ...
                if order_ids:
                    ...
                    return {item['product_id']: item['total_qty'] for item in demand}, 'routes'
            ...
            if pending_orders.exists():
                ...
                return {item['product_id']: item['total_qty'] for item in demand}, 'orders'
        ...
        dispatched_today_orders = Order.objects.filter(
            ...
            status__in=[OrderStatus.DELIVERED, OrderStatus.DISPATCHED, OrderStatus.IN_TRANSIT]
        )
        dispatched_items = OrderItem.objects.filter(
            order__in=dispatched_today_orders
        ).values('product_id').annotate(total_qty=Sum('quantity'))
        dispatched_map = {item['product_id']: item['total_qty'] for item in dispatched_items}

        # Perform Derived Demand Rollups to Raw Materials
        from inventory.models import RawMaterial as RawMaterialModel, Product as ProductModel
        raw_materials = {rm.id: rm for rm in RawMaterialModel.objects.filter(is_active=True)}
        all_products = {p.id: p for p in ProductModel.objects.filter(is_active=True).select_related('bottle_type')}
        ...
        for pid, product in all_products.items():
            ...
            vol_ml = product.bottle_type.volume_ml if (product.bottle_type and product.is_returnable) else 1000
            conversion_factor = float(vol_ml) / 1000.0 if raw_mat.unit.lower() == 'litre' else 1.0

            if pid in today_demands:
                raw_today_demands[rm_id] = raw_today_demands.get(rm_id, 0.0) + (today_demands[pid] * conversion_factor)
            if pid in tomorrow_demands:
                raw_tomorrow_demands[rm_id] = raw_tomorrow_demands.get(rm_id, 0.0) + (tomorrow_demands[pid] * conversion_factor)
            if pid in day_after_demands:
                raw_day_after_demands[rm_id] = raw_day_after_demands.get(rm_id, 0.0) + (day_after_demands[pid] * conversion_factor)
            if pid in dispatched_map:
                raw_dispatched_map[rm_id] = raw_dispatched_map.get(rm_id, 0.0) + (dispatched_map[pid] * conversion_factor)
        ...
        for rm in raw_materials.values():
            ...
            projected_balance = current_stock - pending_today - tomorrow_qty - day_after_qty

            # Order recommendation: how much to procure to stay above reorder level
            order_recommendation = max(0, (pending_today + tomorrow_qty + day_after_qty + reorder_level) - current_stock)
    ...
    def history(self, request, pk=None):
        ...
        """
        warehouse = self.get_object()
        movements = StockMovement.objects.filter(warehouse=warehouse).select_related('raw_material', 'recorded_by')
    ...
    def adjust_stock(self, request, pk=None):
        ...
        raw_material_id = request.data.get('raw_material') or request.data.get('product')
...
class BottleTransactionViewSet(viewsets.ModelViewSet):
    ...
    def summary(self, request):
        ...
        driver_breakdown = []
        routes_today = Route.objects.filter(delivery_date=today).select_related('driver')
        if warehouse_id:
            routes_today = routes_today.filter(warehouse_id=warehouse_id)
        ...
        for route in routes_today:
            ...
            driver_breakdown.append({
                ...
                'driver_warehouse_id': str(route.driver.warehouse.id) if (route.driver and route.driver.warehouse) else None,
                'driver_warehouse_name': route.driver.warehouse.name if (route.driver and route.driver.warehouse) else "No Warehouse",
