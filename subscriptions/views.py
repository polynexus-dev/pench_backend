...
class SubscriptionViewSet(viewsets.ModelViewSet):
    ...
    def pause(self, request, pk=None):
        ...
        if not pause_start or not pause_end:
            return Response(
                {"detail": "pause_start and pause_end are required."}, status=400
            )
    ...
    def update_quantity(self, request, pk=None):
        ...
        if product_id is None or new_quantity is None:
            return Response(
                {"detail": "product_id and quantity are required."}, status=400
            )

        from .models import SubscriptionItem

        item = SubscriptionItem.objects.filter(
            subscription=subscription, product_id=product_id
        ).first()
        if not item:
            return Response(
                {"detail": "Product not found in this subscription."}, status=404
            )
    ...
    def add_skip_date(self, request, pk=None):
        ...
        skip_obj, created = SubscriptionSkipDate.objects.get_or_create(
            subscription=subscription, skip_date=skip_date, defaults={"reason": reason}
        )
    ...
    def monthly_summary(self, request, pk=None):
        ...
        skip_dates = set(
            SubscriptionSkipDate.objects.filter(
                subscription=subscription, skip_date__range=(month_start, month_end)
        ...
        for day_num in range(1, total_days + 1):
            ...
            if day in order_map:
                ...
                elif order_info["order_status"] in [
    ...
    def customer_monthly_summary(self, request):
        ...
        for sub in subscriptions:
            ...
            skip_dates = set(
                sub.skip_dates.filter(
                    skip_date__range=(month_start, month_end)
                ).values_list("skip_date", flat=True)
            )
