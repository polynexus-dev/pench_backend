...
for domain_name, tenant in domains_to_register:
    ...
    if created:
        print(f"  [CREATED] Domain '{domain_name}' mapped to tenant schema '{tenant.schema_name}'")
    else:
        # If it exists but belongs to a different tenant, update it
        if domain_obj.tenant != tenant:
            domain_obj.tenant = tenant
            domain_obj.save()
            print(f"  [UPDATED] Domain '{domain_name}' moved to tenant schema '{tenant.schema_name}'")
        else:
            print(f"  [EXISTS] Domain '{domain_name}' already mapped to tenant schema '{tenant.schema_name}'")
