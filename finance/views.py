...
class MonthlyBillViewSet(viewsets.ModelViewSet):
    ...
    def summary(self, request):
        ...
        collected_today = Transaction.objects.filter(
        ...
        collection_rate = (float(total_collected) / float(total_billed) * 100) if total_billed else 0
    ...
    def download_pdf(self, request, pk=None):
        ...
        class InvoicePDF(FPDF):
            ...
            def header(self):
                ...
                self.cell(w=0, h=5, text="Premium Dairy & Farm Fresh Deliveries", new_x="LMARGIN", new_y="NEXT")
                
                # Invoice Title (Right aligned)
                self.set_xy(130, 12)
                self.set_text_color(255, 255, 255)
                self.set_font("helvetica", "B", 20)
                self.cell(w=65, h=8, text="INVOICE", align="R", new_x="LMARGIN", new_y="NEXT")
            ...
            def footer(self):
                ...
                self.cell(w=0, h=4, text="Thank you for your business! For any billing queries, contact billing@penchfoods.com", align="C", new_x="LMARGIN", new_y="NEXT")
        ...
        pdf.cell(w=90, h=5, text="INVOICE DETAILS:", align="R", new_x="LMARGIN", new_y="NEXT")
        ...
        for i in range(max_lines):
            ...
            # Print right column
            pdf.set_font("helvetica", "B" if i == 0 or "Status" in right_text else "", 10)
            ...
            pdf.cell(w=90, h=5, text=right_text, align="R", new_x="LMARGIN", new_y="NEXT")
        ...
        pdf.cell(w=40, h=8, text="Status", border=1, align="C", fill=True, new_x="LMARGIN", new_y="NEXT")
        ...
        pdf.cell(w=50, h=8, text=f"INR {bill.remaining_amount:,.2f}", border=1, align="C")
        ...
        pdf.cell(w=40, h=8, text=inv_status, border=1, align="C", new_x="LMARGIN", new_y="NEXT")
        pdf.set_text_color(15, 23, 42)  # reset
        ...
        pdf.set_font("helvetica", "B", 12)
        pdf.cell(w=0, h=6, text="Delivered Orders Breakdown", new_x="LMARGIN", new_y="NEXT")
        pdf.ln(2)
        ...
        pdf.cell(w=40, h=6, text="Amount (INR)", border=1, align="R", fill=True, new_x="LMARGIN", new_y="NEXT")
        
        pdf.set_font("helvetica", "", 9)
        pdf.set_text_color(15, 23, 42)
        
        if not orders.exists():
            pdf.cell(w=190, h=6, text="No delivered orders logged for this billing period.", border=1, align="C", new_x="LMARGIN", new_y="NEXT")
        else:
            for order in orders:
                if pdf.get_y() > 250:
                    pdf.add_page()
                ...
                pdf.cell(w=40, h=6, text=f"{order.total:.2f}", border=1, align="R", new_x="LMARGIN", new_y="NEXT")
        ...
        pdf.set_font("helvetica", "B", 12)
        pdf.cell(w=0, h=6, text="Received Payments History", new_x="LMARGIN", new_y="NEXT")
        pdf.ln(2)
        ...
        if not transactions.exists():
            pdf.cell(w=190, h=6, text="No payments recorded against this invoice.", border=1, align="C", new_x="LMARGIN", new_y="NEXT")
        else:
            for tx in transactions:
                ...
                pdf.cell(w=40, h=6, text=f"{tx.amount:.2f}", border=1, align="R", new_x="LMARGIN", new_y="NEXT")
...
class TransactionViewSet(viewsets.ModelViewSet):
    queryset = Transaction.objects.all().select_related('bill__customer').order_by('-payment_date')
