...
def run():
    ...
    for creds in credentials:
        ...
        try:
            ...
            token = res_json.get("access") or res_json.get("token") or res_json.get("tokens", {}).get("access")
            if token:
                print(f"  Success! Logged in as {creds['username']}.")
                print(f"  Response user keys: {res_json.get('user', {}).keys() if 'user' in res_json else 'None'}")
