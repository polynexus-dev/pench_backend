...
with schema_context(city.schema_name):
    ...
    print(f"Created pending order. Status: {order.status}, Delivered At: {order.delivered_at}")
    ...
    print(f"Re-fetched status: {refetched_order.status}, Delivered At: {refetched_order.delivered_at}")
    if refetched_order.delivered_at:
        print("[SUCCESS] Order.save() successfully appended and saved 'delivered_at' inside 'update_fields'!")
    else:
        print("[FAILED] Order.save() FAILED to save 'delivered_at' when using update_fields!")
    ...
    Order.objects.filter(id=order2.id).update(status=OrderStatus.DELIVERED, delivered_at=timezone.now())
    
    refetched_order2 = Order.objects.get(id=order2.id)
    print(f"Re-fetched second order status: {refetched_order2.status}, Delivered At: {refetched_order2.delivered_at}")
    ...
    print(f"Created previous route ID: {prev_route.id} (is_completed={prev_route.is_completed}) with 3 stops.")
    ...
    print(f"Previous route after new route creation: is_completed={prev_route.is_completed}, status={prev_route.status}")
    print(f"  Stop 1 (was Pending) status: {ord_prev1.status}, Delivered At: {ord_prev1.delivered_at}")
    print(f"  Stop 2 (was Undelivered) status: {ord_prev2.status}, Delivered At: {ord_prev2.delivered_at}")
    print(f"  Stop 3 (was In Transit) status: {ord_prev3.status}, Delivered At: {ord_prev3.delivered_at}")
    
    if prev_route.is_completed and ord_prev1.status == OrderStatus.UNDELIVERED and ord_prev3.status == OrderStatus.UNDELIVERED:
        print("[SUCCESS] Previous active route was successfully completed and remaining stops marked undelivered!")
    else:
    ...
    print(f"Undelivered order status after start: {o_undelivered.status} (expected undelivered)")
    
    if o_pending.status == OrderStatus.IN_TRANSIT and o_undelivered.status == OrderStatus.UNDELIVERED:
        print("[SUCCESS] start_trip_for_route successfully preserved the UNDELIVERED status while transitioning pending to in_transit!")
    else:
