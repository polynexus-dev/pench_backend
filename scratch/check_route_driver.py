...
with schema_context("pench-nagpur"):
    ...
    for d in drivers:
        print(
            f"  Driver: {d.user.username} | Warehouse: {d.warehouse.name if d.warehouse else 'None'}"
        )

    print(f"Zones: {len(zones)}")
    for z in zones:
        print(
            f"  Zone: {z.name} | Assigned User: {z.assigned_driver.username if z.assigned_driver else 'None'}"
        )

    print(
        f"Customers: {len(customers)} | Customers with Zones: {Customer.objects.exclude(zone=None).count()}"
    )
    ...
    if driver and not driver.warehouse:
        print(
            f"Associating driver '{driver.user.username}' with warehouse '{warehouse.name}'..."
        )
    ...
    if zone and driver:
        print(
            f"Assigning zone '{zone.name}' to driver user '{driver.user.username}'..."
        )
    ...
    if customer:
        ...
        print(
            f"Customer: {customer.name} | Zone: {customer.zone.name if customer.zone else 'None'} | Location: {customer.location}"
        )
        ...
        for r in generated_routes:
            ...
            if r.geometry:
                ...
                print(
                    f"  Warehouse Coordinate: ({warehouse.longitude}, {warehouse.latitude})"
                )
