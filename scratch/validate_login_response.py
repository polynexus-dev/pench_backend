...
serializer = MyTokenObtainPairSerializer(
    context={"request": FakeRequest("localhost:8888", secure=False)}
)
...
try:
    ...
    if data.get("domain_name") == expected_val and "full_domain" not in data:
        ...
        print(
            f"[FAILED] domain_name mismatch! Expected '{expected_val}', got '{data.get('domain_name')}'"
        )
...
serializer = MyTokenObtainPairSerializer(
    context={"request": FakeRequest("pench.api.polynexus.in", secure=True)}
)
serializer.user = user
...
try:
    ...
    if data.get("domain_name") == expected_val and "full_domain" not in data:
        print(
            "[SUCCESS] domain_name correctly matched https://pench.api.polynexus.in directly!"
        )
    else:
        print(
            f"[FAILED] domain_name mismatch! Expected '{expected_val}', got '{data.get('domain_name')}'"
