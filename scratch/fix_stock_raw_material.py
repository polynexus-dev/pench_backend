    ...
    for schema in schemas:
        ...
        with schema_context(schema):
            with connection.cursor() as cursor:
                ...
                if has_raw_material:
                    print(f"  [i] Schema '{schema}' already has 'raw_material_id' in 'inventory_stock'. Checking product_id...")
                else:
                    print(f"  [*] Adding 'raw_material_id' column to 'inventory_stock' in '{schema}'...")
                ...
                if has_product:
                    print(f"  [*] Migrating stock data from products to raw materials in '{schema}'...")
                    with transaction.atomic():
                        ...
                        try:
                            cursor.execute("ALTER TABLE inventory_stock DROP CONSTRAINT IF EXISTS inventory_stock_product_id_warehouse_id_74744dca_uniq")
                        except Exception as e:
                            print(f"  [i] Unique constraint drop info: {e}")

                        # Drop product_id column
                        cursor.execute("ALTER TABLE inventory_stock DROP COLUMN product_id")
                ...
                try:
                    with transaction.atomic():
                        ...
                        """)
                        print("  [+] Added unique constraint (raw_material_id, warehouse_id).")
                except Exception as e:
                    print(f"  [i] Unique constraint on raw_material_id + warehouse_id already exists or error: {e}")
