...
def run():
    ...
    token = response.json().get("access") or response.json().get("tokens", {}).get(
        "access"
    )
    ...
    # Fetch customers
    cust_res = session.get(
        f"{BASE_URL}/api/erp/customers/", headers=headers, verify=False
    )
    ...
    cust_filter_res = session.get(
        f"{BASE_URL}/api/erp/customers/?qr_code_id=c4ed832a-4844-42ed-8519-d205283f3b8d",
        headers=headers,
        verify=False,
    )
