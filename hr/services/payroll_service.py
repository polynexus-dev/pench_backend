...
from hr.models import (
    Employee, MonthlyPayroll, MonthlyPayrollStatus, 
    EmployeeSalary, DeliveryIncentiveRule
)
...
def generate_monthly_payroll(month, year):
    ...
    for employee in employees:
        ...
        salary_info = EmployeeSalary.objects.filter(
            employee=employee,
        ...
        if not salary_info:
            logger.warning(f"No active salary structure for employee {employee.employee_id}")
            continue
