...
class DriverLocationSerializer(serializers.ModelSerializer):
    ...
    def get_trail(self, obj):
        ...
        active_route = Route.objects.filter(
            driver__user_id=obj.user.id, status=RouteStatus.IN_PROGRESS
        ).first()

        if active_route:
            trails = DriverTrail.objects.filter(
                user=obj.user, route=active_route
            ).order_by("timestamp")
        else:
            import datetime
            ...
            trails = DriverTrail.objects.filter(
                user=obj.user, route__isnull=True, timestamp__date=today
            ).order_by("timestamp")
    ...
    def get_distance_km(self, obj):
        ...
        for i in range(len(trail) - 1):
            total_dist += calc_dist(
                trail[i][0], trail[i][1], trail[i + 1][0], trail[i + 1][1]
            )
...
class DriverLocationViewSet(viewsets.ReadOnlyModelViewSet):
    ...
    def trail(self, request, pk=None):
        ...
        if route_id:
            ...
            if active_route:
                trails = DriverTrail.objects.filter(
                    user=driver, route=active_route
                ).order_by("timestamp")
                ...
                trails = DriverTrail.objects.filter(
                    user=driver, timestamp__date=date
                ).order_by("timestamp")
        ...
        if not routed_trail:
            ...
            geometry = {"type": "Point", "coordinates": routed_trail[0]}
        else:
            geometry = {"type": "LineString", "coordinates": routed_trail}
