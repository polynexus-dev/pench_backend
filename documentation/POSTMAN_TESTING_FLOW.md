...
- **Request**: `02. Platform & Tenants / Cities (City) / POST Create City`
  ...
  - **Payload**:
    ...
    ```
  - *Result*: Copy the returned `id` and save it to `city_id`. A database schema named `nagpur` is automatically created and migrated.

### 4. Create Domain Mapping
- **Request**: `02. Platform & Tenants / Domains (Domain) / POST Create Domain`
  - **Method**: `POST`
  - **Payload**:
    ```json
    {
      "domain": "nagpur.localhost",
      "tenant": "{{city_id}}",
      "is_primary": true
    }
    ```
...
### 18. Generate Monthly Bills (Invoices)
- **Request**: `08. Finance & Billing / bills / POST Trigger Generation`
  - **Method**: `POST`
  - **URL**: `{{city_url}}/api/finance/bills/trigger-generation/`
  - **Payload**:
    ```json
    {
      "year": 2026,
    ...
    ```
- **Request**: `08. Finance & Billing / bills / GET List Bills`
  - **Method**: `GET`
  - **URL**: `{{city_url}}/api/finance/bills/` (You can also filter by customer: `?customer={{customer_id}}` or status: `?status=unpaid`)
