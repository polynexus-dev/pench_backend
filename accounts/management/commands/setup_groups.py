...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        self.stdout.write(
            self.style.SUCCESS(
                f"Synced {users_synced} users to their respective groups."
            )
        )
        ...
        try:
            ...
            if connection.schema_name == "public":
                ...
                self.stdout.write(
                    self.style.SUCCESS(
                        "Successfully configured: Generate Next Day Routes (9:00 PM)"
                    )
                )
                ...
                self.stdout.write(
                    self.style.SUCCESS(
                        "Successfully configured: Auto Stop Trips at 12 PM"
                    )
                )
            else:
            ...
            self.stdout.write(
                self.style.WARNING(f"Failed to set up periodic tasks (non-fatal): {e}")
            )
