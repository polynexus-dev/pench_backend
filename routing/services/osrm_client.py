...
def match_trail(
    coordinates: list[list[float]], radiuses: list[float] = None
) -> list[list[float]]:
    """
    ...
    if len(coordinates) > max_chunk_size:
        ...
        for i in range(0, len(coordinates) - 1, max_chunk_size - 1):
            ...
            chunk_radiuses = radiuses[i : i + max_chunk_size] if radiuses else None
            chunk_route = match_trail(chunk, chunk_radiuses)
    ...
    params = {"overview": "full", "geometries": "geojson", "tidy": "true"}
    ...
    try:
        ...
        if data.get("code") != "Ok":
            raise RuntimeError(
                f"OSRM code not Ok: {data.get('code')}: {data.get('message')}"
            )
