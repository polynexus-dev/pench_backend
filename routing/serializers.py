...
def parse_geojson(content):
    ...
    if geom.geom_type not in ['Polygon', 'MultiPolygon']:
        raise ValueError(f"Geometry must be Polygon or MultiPolygon, got {geom.geom_type}")
    return geom
...
def parse_zip_shapefile(zip_filepath):
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        if not geoms:
            raise ValueError("No valid Polygon or MultiPolygon geometries found in shapefile.")
...
class ZoneSerializer(serializers.ModelSerializer):
    ...
    boundary_file = serializers.FileField(required=False, write_only=True, help_text="Upload a .geojson, .kml, or zipped shapefile (.zip) to set the zone limits.")

    class Meta:
        model = Zone
        fields = ['id', 'name', 'description', 'boundary', 'boundary_file', 'assigned_driver', 'driver_name', 'is_active']
