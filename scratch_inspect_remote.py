...
def run_remote_inspect():
    ...
    for zid, z in zones_by_id.items():
        print(f"Zone Name: {z.get('name')}, ID: {zid}, Driver: {z.get('driver_name')} (ID: {z.get('assigned_driver')})")

    print("\n--- VM CUSTOMERS ---")
    for cid, c in customers_by_id.items():
        zone_id = c.get('zone')
        zone_name = zones_by_id[zone_id].get('name') if zone_id in zones_by_id else 'None'
        print(f"Customer Name: {c.get('name')}, ID: {cid}, Zone: {zone_name} (ID: {zone_id})")
    ...
    if isinstance(orders, list):
        for o in orders:
            ...
            if status in ['pending', 'confirmed']:
                ...
                zone_name = zones_by_id[zone_id].get('name') if zone_id in zones_by_id else 'None'
                print(f"Order ID: {o.get('id')}, Customer: {cust_name}, Zone: {zone_name}, Date: {o.get('scheduled_delivery_date')}, Status: {status}")
    else:
