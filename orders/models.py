...
class Order(BaseModel):
    ...
    pod_longitude = models.DecimalField(
        max_digits=12, decimal_places=9, null=True, blank=True
    )
    ...
    def save(self, *args, **kwargs):
        if (
            self.status in [OrderStatus.DELIVERED, OrderStatus.UNDELIVERED]
...
class OrderItem(BaseModel):
    ...
    product = models.ForeignKey(
        "inventory.Product", on_delete=models.PROTECT, related_name="order_items"
    )
...
class Route(BaseModel):
    ...
    additional_drivers = models.ManyToManyField(
        "accounts.User", related_name="additional_routes", blank=True
    )
...
class RouteStop(BaseModel):
    ...
    order = models.OneToOneField(
        Order, on_delete=models.CASCADE, related_name="route_stop"
    )
...
class DeliveryLog(BaseModel):
    ...
    route = models.ForeignKey(
        Route, on_delete=models.SET_NULL, null=True, blank=True, related_name="logs"
    )
    order = models.ForeignKey(
        Order, on_delete=models.SET_NULL, null=True, blank=True, related_name="logs"
    )
