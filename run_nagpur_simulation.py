        ...
        async with websockets.connect(ws_uri) as ws:
            ...
            while True:
                ...
                if data.get('type') == 'broadcast_location':
                    ...
                    print(f"  Snaps Route points count: {len(data.get('trail', []))}{RESET}")
    except Exception as e:
        pass
...
async def run_simulation(host, port, username, password, interval):
    ...
    if not token:
        print(f"{RED}[!] Aborting. Please check if server is running on port {port}.{RESET}")
        return
    ...
    try:
        async with websockets.connect(ws_uri) as websocket:
            ...
            print(f"{YELLOW}[*] Interval: {interval} seconds per ping. Total points: {len(NAGPUR_COORDINATES)}{RESET}")
            ...
            for coord in NAGPUR_COORDINATES:
                ...
                try:
                    ...
                    if res.get('type') == 'location_update_response':
                        trail = res.get('trail', [])
                        print(f"{GREEN}[<-] Server Acknowledged! Snapped Location: [{res.get('lng')}, {res.get('lat')}]{RESET}")
                        print(f"{GREEN}[<-] Trail coordinates count: {len(trail)} (Successfully snapped along roads!){RESET}")
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description="Nagpur Live WebSockets Tracking Simulator")
    parser.add_argument('--host', type=str, default='nagpur.localhost', help='Tenant host name')
    parser.add_argument('--port', type=str, default='8000', help='Server port')
    parser.add_argument('--username', type=str, default='driver_nagpur_10', help='Driver username')
    parser.add_argument('--password', type=str, default='securepass123', help='Driver password')
    parser.add_argument('--interval', type=float, default=5.0, help='Ping interval in seconds')
    
    args = parser.parse_args()
    
    try:
        asyncio.run(run_simulation(args.host, args.port, args.username, args.password, args.interval))
