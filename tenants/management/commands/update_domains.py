...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        parser.add_argument(
            'new_target',
            type=str,
            help='The new VM IP address or hostname.'
        )
        parser.add_argument(
            '--clean-old',
            type=str,
    ...
    def handle(self, *args, **options):
        ...
        try:
            ...
            raise CommandError("Public tenant (schema_name='public') does not exist in the database.")
        ...
        if is_ip:
            ...
            for tenant in other_tenants:
                domains_to_create_or_update.append((f"{tenant.schema_name}.{new_target}.nip.io", tenant))
        else:
            domains_to_create_or_update.append((new_target, public_tenant))
            for tenant in other_tenants:
                domains_to_create_or_update.append((f"{tenant.schema_name}.{new_target}", tenant))
        ...
        if not no_cleanup:
            ...
            domains_to_delete = [d for d in domains_to_delete if d.domain.lower() not in new_domain_names]
        ...
        for domain_name, tenant in domains_to_create_or_update:
            ...
            if domain_obj:
                if domain_obj.tenant == tenant:
                    self.stdout.write(f"  [EXISTS] Domain '{domain_name}' already mapped to tenant '{tenant.schema_name}'")
                else:
                    if dry_run:
