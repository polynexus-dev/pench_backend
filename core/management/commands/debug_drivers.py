    ...
    def handle(self, *args, **options):
        ...
        with schema_context("public"):
            ...
            with connection.cursor() as cursor:
                cursor.execute(
                    "SELECT count(*) FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'routing_driver'"
                )
                ...
                if exists:
                    ...
                    self.stdout.write(
                        self.style.WARNING(
                            f"Found {count} drivers in PUBLIC schema (This is usually WRONG)."
                        )
                    )
                else:
                    self.stdout.write(
                        self.style.SUCCESS("Public schema is clean of driver data.")
                    )
        ...
        for city in cities:
            with schema_context(city.schema_name):
                ...
                self.stdout.write(
                    self.style.SUCCESS(
                        f"--- City: {city.name} ({city.schema_name}) ---"
                    )
                )
                ...
                for driver in drivers:
                    user_info = (
                        f"{driver.user.username} (ID: {driver.user_id})"
                        if driver.user
                        else "NULL USER"
                    )
                    self.stdout.write(
                        f"  - Driver ID: {driver.id} | User: {user_info} | Plate: {driver.vehicle_plate}"
                    )
