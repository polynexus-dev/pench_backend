...
class CustomerSerializer(serializers.ModelSerializer):
    ...
    def get_product_rates(self, obj):
        ...
        custom_prices = {cp.product_id: cp for cp in CustomerProductPrice.objects.filter(customer=obj)}
    ...
    def update(self, instance, validated_data):
        ...
        if product_rates_data and isinstance(product_rates_data, list):
            ...
            for rate_data in product_rates_data:
                ...
                if discount <= 0:
                    CustomerProductPrice.objects.filter(customer=updated_instance, product=product).delete()
                else:
                    CustomerProductPrice.objects.update_or_create(
